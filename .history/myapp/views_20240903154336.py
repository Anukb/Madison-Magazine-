...
def register_view(request):
    if request.method == 'POST':
        ...
        if password != confirm_password:
            return render(request, 'accounts/login.html', {'error_message': 'Passwords do not match'})
        
        if User.objects.filter(username=username).exists():
            return render(request, 'accounts/login.html', {'error_message': 'Username already exists'})
